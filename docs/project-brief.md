...
- One reference kit is fully built; additional kits are produced on demand via
  `docs/factory/RUN-A-NEW-KIT.md`. The reusable machine (docs, templates,
  validator, Rancher MCP) is in place so a new kit is mostly assembly, not
  invention.
- The portal's console (the root page at edge-ai-demo.vercel.app; old `/demo`
  URLs redirect to it) is a **labeled presentation/reach layer**, not a
